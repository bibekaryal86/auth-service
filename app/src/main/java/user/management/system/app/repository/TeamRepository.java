...
public class TeamRepository {
  ...
  private static final String SQL_GET_ALL_TEAMS =
      "WITH LimitedTeams AS ( "
          ...
          + "LimitedTeams t "
          + "LEFT JOIN "
          + "users_roles_teams url ON t.id = urt.team_id "
          + "LEFT JOIN "
          + "users u ON urt.user_id = u.id AND (:includeDeletedUsers = TRUE OR users.deleted IS NULL)"
          + "LEFT JOIN "
          + "roles r ON urt.role_id = r.id AND (:includeDeletedRoles = TRUE OR roles.deleted IS NULL)"
          + "ORDER BY "
          + "t.name";
